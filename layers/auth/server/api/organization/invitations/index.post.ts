...
export default defineAuthorizedHandler(['user:manage'], async (event, { session }) => {
  ...
  const { email, role_id, metadata } = await readValidatedBody(event, CreateInvitationSchema.parse)
  ...
  return createInvitation(orgId, email, role_id ?? null, session.id, metadata ?? {})
})
