...
export const userTable = sqliteTable('users', {
  ...
  bio: text('bio'),
  notification_prefs: text('notification_prefs', { mode: 'json' }).$type<{
    email: boolean
...
export const identityRelations = relations(identityTable, ({ one }) => ({
  user: one(userTable, { fields: [identityTable.user_id], references: [userTable.id] }),
}))
export const activityRelations = relations(activityTable, ({ one }) => ({
  user: one(userTable, { fields: [activityTable.user_id], references: [userTable.id] }),
}))
