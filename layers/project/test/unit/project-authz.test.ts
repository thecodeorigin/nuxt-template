...
describe('gate constants invariant', () => {
  it('write-targeting gates never admit the unscoped create key (project:write)', () => {
    expect(['project:write:self', 'project:manage']).not.toContain('project:write')
    expect(['project:delete', 'project:delete:self', 'project:manage']).not.toContain('project:write')
  })

  it('only the create gate lists unscoped project:write', () => {
    expect(['project:write', 'project:manage']).toContain('project:write')
  })
})
...
describe('delete gate — owner vs non-owner vs admin', () => {
  it('admin (manage) deletes any project', async () => {
    ...
    const r = await evaluateChecks(['project:delete', 'project:delete:self', 'project:manage'], makeSession({ id: 'admin-1', abilities: ADMIN }), makeEvent({ id: 'p-1' }))
    expect(r.allowed).toBe(true)
  })

  it('member deletes a project they own', async () => {
    registerOwner('user-1')
    const r = await evaluateChecks(['project:delete', 'project:delete:self', 'project:manage'], makeSession({ id: 'user-1', abilities: MEMBER }), makeEvent({ id: 'p-1' }))
    expect(r.allowed).toBe(true)
  })
  ...
  it('guest cannot delete', async () => {
    registerOwner('user-1')
    const r = await evaluateChecks(['project:delete', 'project:delete:self', 'project:manage'], makeSession({ id: 'user-1', abilities: GUEST }), makeEvent({ id: 'p-1' }))
    expect(r.allowed).toBe(false)
  })
})
...
describe('write-self gate (patch / members / products)', () => {
  it('owner passes', async () => {
    registerOwner('user-1')
    const r = await evaluateChecks(['project:write:self', 'project:manage'], makeSession({ id: 'user-1', abilities: MEMBER }), makeEvent({ id: 'p-1' }))
    expect(r.allowed).toBe(true)
  })
  ...
  it('admin passes without ownership', async () => {
    registerOwner('owner-2')
    const r = await evaluateChecks(['project:write:self', 'project:manage'], makeSession({ id: 'admin-1', abilities: ADMIN }), makeEvent({ id: 'p-1' }))
    expect(r.allowed).toBe(true)
  })
})
...
describe('create gate', () => {
  it('member creates (unscoped write, no resource id)', async () => {
    const r = await evaluateChecks(['project:write', 'project:manage'], makeSession({ abilities: MEMBER }), makeEvent())
    expect(r.allowed).toBe(true)
  })

  it('guest cannot create', async () => {
    const r = await evaluateChecks(['project:write', 'project:manage'], makeSession({ abilities: GUEST }), makeEvent())
    expect(r.allowed).toBe(false)
  })
})
...
describe('read gates (org-wide read kept)', () => {
  it('member reads list and detail via unscoped read', async () => {
    registerOwner('owner-2')
    expect((await evaluateChecks(['project:read', 'project:manage'], makeSession({ abilities: MEMBER }), makeEvent())).allowed).toBe(true)
    expect((await evaluateChecks(['project:read', 'project:read:self', 'project:manage'], makeSession({ abilities: MEMBER }), makeEvent({ id: 'p-1' }))).allowed).toBe(true)
  })
})
